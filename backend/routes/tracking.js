...
function getOrderTimeline(order) {
  ...
  timeline.push({
    ...
    date: isProcessing ? (order.processing_at || order.updated_at) : null,
  ...
  timeline.push({
    status: 'shipped',
    label: 'Shipped',
    date: isShipped ? (order.shipped_at || order.updated_at) : null,
    completed: isShipped,
    description: isShipped ? 'Your order is on the way' : 'Order will be shipped soon'
  });
  
  // Out for Delivery (optional intermediate step)
  ...
  timeline.push({
    status: 'delivered',
    label: 'Delivered',
    date: isDelivered ? (order.delivered_at || order.updated_at) : null,
    completed: isDelivered,
    description: isDelivered ? 'Order has been delivered successfully' : 'Estimated delivery in 3-5 days'
  });
